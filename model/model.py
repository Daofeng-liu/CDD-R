...
class Model:
    ...
    def __init__(self, x_train, y_, num_tasks, opt, imp_method, synap_stgth, fisher_update_after, fisher_ema_decay, m1,
        ...
        if imp_method == 'PNN':
            ...
            if (
                    imp_method == 'A-GEM' or imp_method == 'ER' or imp_method == 'MEGA' or imp_method == 'MEGAD' or imp_method == 'MEGA_RA' or imp_method == 'CDD' ) and 'FC-' not in network_arch:  # Only for Split-X setups
                self.output_mask = [tf.placeholder(dtype=tf.float32, shape=[self.total_classes]) for i in range(num_tasks)]
        ...
        self.org_feat = tf.placeholder(tf.float32, shape=[None, 1024], name='org_features')
        self.kl_loss_save = tf.placeholder(dtype=tf.float32, shape=())
        self.cd_loss_save = tf.placeholder(dtype=tf.float32, shape=())
        ...
        self.org_logits = tf.placeholder(tf.float32, shape=[None, self.total_classes], name='org_logits')
        self.masks = tf.placeholder(tf.float32, shape=[None, self.total_classes], name='masks')
        self.flag1 = tf.placeholder(dtype=tf.int32, shape=(), name='flag1')
        if network_arch == 'RESNET-B' and self.total_classes == 200:
    ...
    def loss_and_train_ops_for_one_hot_vector(self, x, y_):
        ...
        if self.network_arch == 'FC-S':
            ...
            if self.imp_method == 'PNN':
                ...
                for i in range(self.num_tasks):
                    ...
                    #                        lambda: tf.reduce_mean(tf.abs(self.org_feat - self.features)))
                    self.KL_loss = tf.cond(tf.equal(self.flag1, 0), lambda: 0.0,
                                           lambda: tf.reduce_mean(tf.abs(self.org_logits - self.logits)))
            elif self.imp_method == 'CDD':
                ...
                for i in range(self.num_tasks):
                    ...
                    marginal_logits = tf.cos(theta)  # (10, 100)
                    marginal_logits_2 = tf.cos(theta)  # (10, 100)
                    ...
                    adjusted_entropy = tf.reduce_sum(tf.cast(tf.tile(tf.equal(self.output_mask[i][None, :], 1.0), [tf.shape(y_)[0], 1]),
                                dtype=tf.float32) * y_, axis=1) * cross_entropy
                    self.unweighted_entropy.append(tf.reduce_sum(adjusted_entropy))  # We will average it later on
                    self.KL_loss = tf.cond(tf.equal(self.flag1, 0), lambda: 0.0,
                                           lambda: tf.reduce_mean(tf.abs(self.org_feat - self.features)))
        ...
        if (self.imp_method != 'PNN') and ((
                                                   self.imp_method != 'A-GEM' and self.imp_method != 'ER' and self.imp_method != 'MEGA' and self.imp_method != 'MEGAD' and self.imp_method != 'MEGA_RA' and self.imp_method != 'CDD') or 'FC-' in self.network_arch):
            ...
            if self.imp_method == 'CDD':
                ...
                self.thetaww = (180 / 3.1415926) * tf.acos(
                    tf.reduce_sum(tf.where(tf.equal(y_, 1.0), self.pruned_logits, tf.zeros_like(self.pruned_logits)),
                                  -1))
                marginal_logits_2 = tf.cos(theta)
                ...
                self.unweighted_entropy = tf.reduce_mean(
                    tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=20 * final_logits))
        ...
        if 'GEM' not in self.imp_method and 'MEGA' not in self.imp_method and 'MEGAD' not in self.imp_method and 'CDD' not in self.imp_method:
        ...
        if self.imp_method == 'EWC':
            ...
            self.create_stochastic_cdd_ops()
        ...
        if (self.imp_method == 'PNN') or ((
                                                  self.imp_method == 'A-GEM' or self.imp_method == 'ER' or self.imp_method == 'MEGA' or self.imp_method == 'MEGAD' or self.imp_method == 'MEGA_RA' or self.imp_method == 'CDD') and 'FC-' not in self.network_arch):
    ...
    def fc_feedforward(self, h, weights, biases, apply_dropout=False):
        ...
        if self.imp_method == 'CDD':
            ...
            self.avg_features = tf.reduce_mean(h, axis=0)
            # # RACL can't
    ...
    def resnet18_conv_feedforward(self, h, kernels, filters, strides):
        ...
        # Apply average pooling
        h = tf.reduce_mean(h, [1, 2])
        print('h(average pooling):-->', h.shape)
    ...
    def loss_and_gradients(self, imp_method):
        ...
        if imp_method == 'PNN':
        ...
        elif imp_method != 'A-GEM' and imp_method != 'MEGA' and imp_method != 'MEGAD' and imp_method != 'MEGA_RA' and imp_method != 'CDD':  # For A-GEM we will define the losses and gradients later on
            if imp_method == 'ER' and 'FC-' not in self.network_arch:
                ...
                self.der_loss = self.reg_loss + self.kl_loss_save + self.cd_loss_save
                # self.der_gradients_vars = self.opt.compute_gradients(self.der_loss, var_list=self.trainable_vars)
            else:
            ...
            self.reg_gradients_vars = self.opt.compute_gradients(self.reg_loss,
                                                                 var_list=self.trainable_vars)
            self.der_gradients_vars = self.opt.compute_gradients(self.der_loss,
                                                                 var_list=self.trainable_vars)
    def train_op(self):
        """
        ...
        if self.imp_method == 'VAN' or self.imp_method == 'ER' :
            ...
            self.der_train = self.opt.apply_gradients(self.der_gradients_vars)
        ...
        for v in range(len(self.trainable_vars)):
            ...
            if self.imp_method == 'MAS':
                ...
                self.hebbian_score_vars.append(
                    tf.Variable(tf.zeros(self.trainable_vars[v].get_shape()), trainable=False))
            elif self.imp_method == 'A-GEM' or self.imp_method == 'S-GEM' or self.imp_method == 'MEGA' or self.imp_method == 'MEGAD' or self.imp_method == 'MEGA_RA' or self.imp_method == 'CDD':
    ...
    def create_stochastic_gem_ops(self):
        ...
        # print("!!!!!!!!!!!!!!!!",range(self.num_tasks))
        ref_grads = tf.gradients(self.agem_loss, self.trainable_vars)

        self.ref_grads_tsne = ref_grads[-1]
        print("self.ref_grads_tsne", self.ref_grads_tsne)
        # Reference gradient for previous tasks
        # print("self.agem_loss",self.agem_loss)
        print("self.trainable_vars",self.trainable_vars)
        # print("ref_grads",len(ref_grads),ref_grads[-1])
        # # print("self.ref_grads",len(self.ref_grads),self.ref_grads[-1])
        # for i in range(62):
        #     print("tf.assign",i,tf.assign(self.ref_grads[i], ref_grads[i]))
    ...
    def create_stochastic_cdd_ops(self):
        """
        """
        ...
        # KL_grads = tf.gradients(self.KL_loss + self.cd_loss_save, self.trainable_vars)
