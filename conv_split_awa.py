...
from utils.utils import average_fgt_stats_across_runs, average_ltr_across_runs, der_update_reservior
...
MODELS = ['VAN', 'PI', 'EWC', 'MAS', 'RWALK', 'A-GEM', 'MEGA', 'MEGAD', 'CDD'] #List of valid models
IMP_METHOD = 'VAN'
SYNAP_STGTH = 75000
...
def train_task_sequence(model, sess, saver, datasets, cross_validate_mode, train_single_epoch, do_sampling, is_herding,  
    ...
    for runid in range(num_runs):
        ...
        if model.imp_method == 'ER':
            sv_knn_buffer = SVKNNBuffer(args, episodic_mem_size, [224, 224, 3], 850, 10)

        if model.imp_method == 'A-GEM' or model.imp_method == 'MEGA' or model.imp_method == 'MEGAD' or model.imp_method == 'CDD':
            ...
            episodic_masks = np.zeros([episodic_mem_size, model.num_tasks*TOTAL_CLASSES])
            episodic_features = np.zeros([episodic_mem_size, 512])
            norm_episodic_features = np.zeros([episodic_mem_size, 512])
            ...
            episodic_logits = np.zeros([episodic_mem_size, model.num_tasks*TOTAL_CLASSES])
        ...
        for task in range(len(task_labels)):
            ...
            final_train_labels = np.zeros([100, model.total_classes])
            ...
            for iters in range(num_iters):
                ...
                if model.imp_method == 'VAN':
                    ...
                    if task == 0:
                        ...
                        # feed_dict[model.cd_loss_save] = dist_loss
                        _, loss = sess.run([model.train_subseq_tasks, model.agem_loss], feed_dict=feed_dict)
                    ...
                    for er_x, er_y_ in zip(train_x[offset:offset + residual], train_y[offset:offset + residual]):
                        ...
                        logit_mask_dict = {m_t: i_t for (m_t, i_t) in zip(model.output_mask, a_gem_logit_mask)}
                        feed_dict.update(logit_mask_dict)
                        final_labels = np.zeros([1, model.num_tasks * TOTAL_CLASSES])
                        a = task*50
                        final_labels[:1, a:a+50] = er_y_
                        feature_c = sess.run(model.features,
                                             feed_dict={
                                                 model.x: np.array([er_x]),
                                                 model.y_: final_labels,
                                                 model.flag1: 0,
                                                 model.keep_prob: 1.0,
                        ...
                        a = task * 50
                        episodic_labels[mem_index, a:a+50] = er_y_
                        ...
                        if len(distances) == 0:
                            ...
                            centroids[cls][indices[min_d]] = np.add(
                                np.multiply(len(temp_labels[cls][indices[min_d]]), centroids[cls][indices[min_d]]),
                                feature_c)
            ...
            if task < (len(task_labels) - 1):
                ...
                if 'GEM' in model.imp_method or 'MEGA' in model.imp_method or 'MEGAD' in model.imp_method or 'CDD' in model.imp_method:
                    data_to_sample_from = {
                    ...
                    if model.imp_method == 'S-GEM':
                    ...
                    elif model.imp_method == 'A-GEM' or model.imp_method == 'MEGA' or model.imp_method == 'MEGAD' or model.imp_method == 'CDD':
                        # Do the uniform sampling/ only get examples from current task
                        importance_array = np.ones(num_train_examples, dtype=np.float32)
                        if KEEP_EPISODIC_MEMORY_FULL:
                        ...
                        if not KEEP_EPISODIC_MEMORY_FULL: # Fill the memory to always keep M/T samples per task
                            ...
                            episodic_masks[eps_offset:eps_offset+total_imp_samples, head_offset:head_offset+TOTAL_CLASSES] = np.ones([total_imp_samples,50])
                            ...
                            # cur_logits = sess.run(model.logits,
                            #                       feed_dict={
                            #                           model.x: episodic_images[eps_offset:eps_offset + total_imp_samples],
                            #                           model.y_: episodic_labels[
                            #                                   eps_offset:eps_offset + total_imp_samples],
                            ...
                            #                           model.keep_prob: 1.0,
                            #                           model.train_phase: False})
                            # episodic_logits[eps_offset:eps_offset + total_imp_samples] = cur_logits
            ...
            if model.imp_method == 'CDD':
                centroid_zero = [100000 for x in range(512)]
                centroids_a = []
                for c, cls in enumerate(task_labels[task]):
                    num = c * SAMPLES_PER_CLASS + 100 * task
                    sum_total = 0
                    pro_centroids = []
                    ...
                    for i in range(SAMPLES_PER_CLASS):
                        ...
                        for j in range(len(centroids[cls])):
                            ...
                            if temp > r:
                                ...
                                a = 50*task
                                episodic_labels[num + i, a:a+50] = temp_labels[cls][j][min_dd]
                    ...
                    # print('xunlian1', a_cls)
                    for i in range(len(a_cls)):
                        centroids_a.append(
                            np.mean(norm_episodic_features[num:num + SAMPLES_PER_CLASS][confident[num:num + SAMPLES_PER_CLASS] == a_cls[i]], axis=0))
...
def main():
    ...
    global synap_stgth_list, learning_rate_list
    args = get_arguments()
    ...
    for imp_method in models_list:
        ...
        elif imp_method == 'A-GEM' or imp_method == 'MEGA' or imp_method == 'MEGAD' or imp_method == 'CDD':
            synap_stgth_list = [0]
            if args.online_cross_val or args.cross_validate_mode:
                pass
            else:
