...
from utils.utils import get_sample_weights, sample_from_dataset, update_episodic_memory_with_less_data, concatenate_datasets, samples_for_each_class, sample_from_dataset_icarl, load_task_specific_data,update_reservior
...
def train_task_sequence(model, sess, saver, datasets, cross_validate_mode, train_single_epoch, do_sampling, is_herding,
    ...
    for runid in range(num_runs):
        ...
        episodic_mem_size = SAMPLES_PER_CLASS * total_classes
        ...
        if model.imp_method == 'ER':
            sv_knn_buffer = SVKNNBuffer(args, episodic_mem_size, [224, 224, 3], 200, EPS_MEM_BATCH_SIZE)
        ...
        if model.imp_method == 'A-GEM' or model.imp_method == 'MEGA' or model.imp_method == 'MEGAD' or model.imp_method == 'CDD':
            ...
            episodic_logits = np.zeros([episodic_mem_size, TOTAL_CLASSES])
        ...
        for task in range(len(task_labels)):
            ...
            for iters in range(num_iters):
                ...
                if model.imp_method == 'VAN':
                    ...
                    for er_x, er_y_ in zip(train_x[offset:offset + 16], train_y[offset:offset + 16]):
                        ...
                        if len(distances) == 0:
                            ...
                            if len(temp_labels[cls][indices[min_d]]) < 10:
            ...
            if task < (len(task_labels) - 1):
                ...
                if 'GEM' in model.imp_method or 'MEGA' in model.imp_method or 'MEGAD' in model.imp_method or 'CDD':
                    data_to_sample_from = {
            ...
            if model.imp_method == 'CDD':
                ...
                for c, cls in enumerate(task_labels[task]):
                    num = c * SAMPLES_PER_CLASS + 50 * task
                    sum_total = 0
                    pro_centroids = []
                    sum_b = 0
...
def main():
    ...
    data_labs=iter(data_labs)
    #print(isinstance(data_labs, Iterable))
    #print(isinstance(data_labs, Iterator))
